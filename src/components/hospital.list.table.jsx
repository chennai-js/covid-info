  ...
  return (
    <Card className="hospitalList my-2 flex-1 overflow-y-auto">
      <div className="list-header grid grid-cols-8 md:grid-cols-6 lg:grid-cols-6 gap-3 mb-3 sticky -top-4 bg-white pb-3 pt-3">
        ...
        <p
          className="cursor-pointer col-span-2 md:col-auto lg:col-auto"
          onClick={() => setSortTypeSelected("nonOxygenV")}
        >
          {`Without Oxygen ${sortTypeSelected === "nonOxygenV" ? "▼" : ""}`}
        </p>
        <p
          className="cursor-pointer col-span-2 md:col-auto lg:col-auto"
          onClick={() => setSortTypeSelected("oxygenV")}
        >
          {`With Oxygen ${sortTypeSelected === "oxygenV" ? "▼" : ""}`}
        </p>
        <p
          className="cursor-pointer col-span-2 md:col-auto lg:col-auto"
          onClick={() => setSortTypeSelected("icuV")}
        >
          {`ICU ${sortTypeSelected === "icuV" ? "▼" : ""}`}
        </p>
        <p
          className="cursor-pointer col-span-2 md:col-auto lg:col-auto"
          onClick={() => setSortTypeSelected("ventilatorV")}
        >
          {`ICU With Ventilator ${
            sortTypeSelected === "ventilatorV" ? "▼" : ""
          }`}
        </p>
      </div>
      {sortedHospitalList.map((hospitalInfo) => {
        return (
          <HospitalsInfo
...
function HospitalsInfo({ hospitalInfo }) {
  ...
  let [icuBedChipType] = useState(() => {
    let percentile =
      (parseInt(hospitalInfo["ICU BEDS Vacant"]) /
  ...
  return (
    <div
      className="list-content grid grid-cols-6 gap-3 hover:bg-gray-200 p-0 pt-3 pb-3 md:p-3 lg:p-3  cursor-pointer"
      onClick={() => toggleAdditionalInfo(!showAdditionalInfo)}
    >
      <p className="col-span-8 md:col-span-2 lg:md:col-span-2 text-green-700 text-lg">
        {hospitalInfo["Institution "]}
      </p>
